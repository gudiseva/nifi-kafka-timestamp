 ...
 */
package nag.arvind.gudiseva.processors.kafka;
...
import static nag.arvind.gudiseva.processors.kafka.KafkaProcessorUtils.HEX_ENCODING;
import static nag.arvind.gudiseva.processors.kafka.KafkaProcessorUtils.UTF8_ENCODING;
...
public class ConsumeKafkaTimestamp_0_10 extends AbstractProcessor {
    ...
    protected ConsumerPool createConsumerPool(final ProcessContext context, final ComponentLog log) {
        ...
        final byte[] demarcator = context.getProperty(ConsumeKafkaTimestamp_0_10.MESSAGE_DEMARCATOR).isSet()
                ? context.getProperty(ConsumeKafkaTimestamp_0_10.MESSAGE_DEMARCATOR).evaluateAttributeExpressions().getValue().getBytes(StandardCharsets.UTF_8)
                : null;
        ...
        final String topicListing = context.getProperty(ConsumeKafkaTimestamp_0_10.TOPICS).evaluateAttributeExpressions().getValue();
        final String topicType = context.getProperty(ConsumeKafkaTimestamp_0_10.TOPIC_TYPE).evaluateAttributeExpressions().getValue();
